document.addEventListener('DOMContentLoaded', (event) => {
    ...
    function applyZoom() {
        if (appState.currentView === 'chat') {
            scriptAreaWrapper.style.transform = `scale(${appState.chatZoom})`;
        } else {
            if (mindmapContentWrapper) {
    ...
    function loadState() {
        ...
        if (storedState) {
            ...
            addBlockToState({ title: "Начало", text: "Дважды кликните, чтобы редактировать этот сценарий. Правый клик для меню." });
        }
    ...
    function addBlockToState(data, parentId = null, direction = "down") {
        ...
        const newBlock = { id: newId, title: data.title || "", text: data.text || "",
            parent: parentId, children: [], direction: direction, x: x, y: y };
    ...
    function renderChat() {
        ...
        appState.blocks.forEach(block => {
            scriptArea.appendChild(createBlockElement(block, 'chat'));
        });
        if (appState.currentView === 'chat') {
    ...
    function renderMindmap() {
        ...
        appState.blocks.forEach(block => {
            mindmapContentWrapper.appendChild(createBlockElement(block, 'map'));
        });
        drawMapLines();
    }
    ...
    function drawMapLines() {
        // Заглушка, если код рисования линий еще не реализован
    }

    function addChildBlock(parentId, direction = "down") {
        addBlockToState({ title: "Новый блок", text: "Введите текст..." }, parentId, direction);
    }
    ...
    function dragBlock(e, blockElement, mapBlock) {
        ...
        function moveAt(pageX, pageY) {
            mapBlock.x = pageX / appState.mapZoom - shiftX;
            mapBlock.y = pageY / appState.mapZoom - shiftY;
